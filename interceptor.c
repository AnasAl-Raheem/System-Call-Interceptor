...
void my_exit_group(int status) {
    // delete PID fromm all syscalls and call original exit function
...
asmlinkage long interceptor(struct pt_regs reg) {
    ...
    // log a message if PID is monitered for syscall, or if all PIDs are monitored
    if (table[reg.ax].monitored == 2 || (table[reg.ax].monitored == 1 
        && check_pid_monitored(reg.ax, current->pid) != 0)) {
        log_message(current->pid, reg.ax, reg.bx, 
            reg.cx, reg.dx, reg.si, reg.di, reg.bp);
    }
    spin_unlock(&pidlist_lock);
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
    ...
    if (cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE) {
        ...
        // only root can perform these commands
        if (current_uid() != 0){
            return -EPERM;
        }
        if (cmd == REQUEST_SYSCALL_INTERCEPT) {
            ...
            // can not intercept a syscall that is already intercepted
            ...
            // change system call table function to interceptor
        ...
        if (cmd == REQUEST_SYSCALL_RELEASE) {
            ...
            // can not release a non intercepted syscall
            if (table[syscall].intercepted == 0) {
                spin_unlock(&pidlist_lock);
                return -EINVAL;
            }
            
            // remove monistered PIDs, if any
            ...
            // restore original function to system call table
            spin_lock(&calltable_lock);
            set_addr_rw((unsigned long)sys_call_table);
    ...
    if (cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING ){
        ...
        // only root or owner of PID can start/stop monitoring
        if (current_uid() != 0 && (pid == 0 
            || check_pid_from_list(pid,current->pid) != 0)) {
            spin_unlock(&pidlist_lock);
            return -EPERM;
        }
        spin_unlock(&pidlist_lock);
        ...
        if (cmd == REQUEST_START_MONITORING) {
            ...
            // PID already monitered by syscall
        ...
        if (cmd == REQUEST_STOP_MONITORING) {
            ...
            // stop moniteroring all PIDs, only if already being monitored
            ...
            // if pid not monitered or sys call not monitered
            if (check_pid_monitored(syscall, pid) == 0  
                || table[syscall].monitored == 2 || pid == 0) {
                spin_unlock(&pidlist_lock);
                return -EINVAL;
            }
...
static int init_function(void) {
    ...
    // initialize each function list, and store original function from system call table
    spin_lock(&pidlist_lock);
...
static void exit_function(void) {        
    ...
    //destroy all monitered PID lists
    spin_lock(&pidlist_lock);
    ...
    // restore original function calls to system call table
    spin_lock(&calltable_lock);
    set_addr_rw((unsigned long)sys_call_table);
    for (i = 1; i < NR_syscalls + 1; i++) {
